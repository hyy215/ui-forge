...
`ui-forge.delivery.read` 从已登记任务的固定产物位置读取报告，位置由 `deliveryReportPath` 按项目临时目录和任务 ID 哈希计算，不接受客户端指定报告路径，也不扫描或猜测最新报告。`deliveryContext` 每轮提供模板和路径，Codex 在原有授权内自行写入；服务不运行验证、不改变任务终态。

哈希路径缺失时仅兼容同一任务临时目录下的 `delivery/report.json`，仍校验报告 `taskId`；直路径中的无指纹证据只保留展示并标记为不可核验，不读取其路径或扩大任务文件范围。

新报告必须使用带 SHA-256 的文件证据对象。为兼容早期任务，读取到裸路径证据时会保留报告并标记为“不支持核对”，不会读取该路径或将其视为匹配。

[delivery/](src/delivery/) 对报告、源码与文件证据进行有界读取和路径校验。源码仅限工作区，证据仅限工作区或该任务交付目录；内容指纹只证明当前所列文件一致。原生证据按主线程 `turnId/itemId` 查找，缺失或读取失败保持不可核对；不恢复线程、不检查 Vibe 画布。命令退出 0 只显示工具成功，不推断模型声明的业务结论成立。缺报告、损坏、越权、过期分别呈现，保留历史声明供人工判断。

交付目录按任务索引保存的原工作区身份定位。工作区改名、删除或重指向其他目录时，只要原 runtime 报告仍可读取，就保留其声明；源码与工作区证据显示不可核对，原生证据不触发连接。仅本任务交付目录中的文件证据可继续核对，不搜索新工作区、不修改绑定、不扩大文件范围。原 runtime 本身不可访问时报告读取失败，不宣称报告不存在。
