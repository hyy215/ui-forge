...
describe("agent-server architecture boundaries", () => {
  it("keeps D2C infrastructure construction inside the composition root", () => {
    const violations = listRuntimeSourceFiles(srcRoot).flatMap((path) => {
      ...
      return imports
        .filter((specifier) => infrastructurePackages.some((prefix) => (
  ...
  it("keeps production HTTP transport independent from the D2C domain package", () => {
    const httpRoot = join(srcRoot, "http");
    const violations = listRuntimeSourceFiles(httpRoot).flatMap((path) => (
      importedSpecifiers(readFileSync(path, "utf8"))
        .filter((specifier) => (
          specifier === "@ui-forge/d2c-agent"
          || specifier.startsWith("@ui-forge/d2c-agent/")
        ))
