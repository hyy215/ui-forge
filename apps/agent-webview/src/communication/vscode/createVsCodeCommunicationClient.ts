...
export function createVsCodeCommunicationClient(
  ...
  return {
    ...
    request: ({ method, params, responseSchema, signal, timeoutMs = 10_000 }) =>
    ...
    stream: ({ method, params, eventSchema, onEvent, signal, timeoutMs = 0 }) =>
      new Promise((resolve, reject) => {
        requestSequence += 1;
        ...
        async function consumeMessage(
          message: ReturnType<typeof communicationStreamMessageSchema.parse>,
        ) {
          if (settled) return;
        ...
        function handleMessage(event: MessageEvent<unknown>) {
          ...
          processing = processing
            .then(() => consumeMessage(messageResult.data))
            .catch((error: unknown) => {
              cancelHostStream();
              cleanup();
              reject(error);
