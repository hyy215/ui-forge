...
describe("VS Code communication client", () => {
  ...
  it("cancels upstream on a validation failure and ignores already queued events", async () => {
    const sentMessages: unknown[] = [];
    const targetWindow = Object.assign(new EventTarget(), {
      acquireVsCodeApi: () => ({ postMessage: (message: unknown) => sentMessages.push(message) }),
      setTimeout: globalThis.setTimeout,
      clearTimeout: globalThis.clearTimeout,
    }) as unknown as Window;
    const events: unknown[] = [];
    const stream = createVsCodeCommunicationClient(targetWindow).stream({
      method: "test",
      eventSchema: z.number(),
      onEvent: (event) => {
        events.push(event);
      },
    });
    ...
    for (const [seq, event] of [
      ...
      targetWindow.dispatchEvent(
        new MessageEvent("message", {
          data: { kind: "stream-event", requestId: request.requestId, seq, event },
        }),
      );
    await expect(stream).rejects.toThrow("领域事件格式无效");
    expect(sentMessages[1]).toEqual({
      kind: "notification",
      method: communicationTransportMethods.cancelStream,
      params: { requestId: request.requestId },
