...
当前版本跑通设计读取、仓库证据驱动的审阅型规划与候选代码生成：用户在单一对话视图中输入 Design URL，系统读取并缓存 MasterGo 设计上下文，从官方矢量与布局数据确定性合成安全 SVG 预览。用户检查右侧预览并在对话中精确回复“确认设计”后，Graph 首先确定性检查目标项目；不支持的项目立即结束，空目录进入初始化规划，React + Ant Design 项目继续生成组件候选并受控扫描源码。主 Plan Agent 委派独立视觉 Subagent 形成布局、组件、可见元素、静态状态和交互理解，再结合仓库与设计系统证据生成复用决策、文件影响及原子实施步骤。Graph 在 Plan 后再次暂停，只有用户明确点击“按此方案生成代码”才会重新读取计划文件、核验内容哈希并由受限 Code Agent 生成逐步骤结构化 Patch。候选 Patch 仅供审阅，不会写入目标项目；反馈修订、受控应用、执行验证和交付仍属于后续能力。
...
flowchart LR
    ...
    W --> X[绑定 Plan 与文件哈希并展示候选 Patch]
```

## 包边界

- `packages/agent-core`：领域无关的 Agent、受限 Deep Agent 与 LangGraph 封装。
- `packages/d2c-agent`：D2C 任务、设计与项目检查领域端口、版本化 Plan、视觉 Subagent、主 Plan Agent、受限 Code Agent、结构化 Patch、Graph 和对外 D2C Service。
...
- `apps/agent-webview`：在单一对话视图中完成 Design URL 输入、SVG 检查、项目校验、方案审阅、显式代码生成确认和候选 Patch Diff 审阅。
...
`packages/d2c-agent/src/planning` 通过 `planVersion` 与 `planHash` 把候选 Patch 绑定到精确 Plan 内容；每个步骤 Patch 还记录变换前后的文件内容哈希。未来通过 `PlanDelta` 调整计划时，受影响步骤的旧 Patch 绑定会失效，人工锁冲突必须返回人工决定。
...
- Code Agent 不拥有文件系统、Shell、网络、写入或子 Agent 能力；只能读取工作流提供的计划文件和明确复用参考文件快照。代码生成前重新校验规范化路径、真实路径、符号链接、文件类型、内容大小和 SHA-256，输出必须逐步骤匹配 Plan 的路径与动作。
- 公开快照只包含审阅 Diff 和哈希，不包含待写入的完整内部 Patch 内容；当前版本没有 Patch 应用命令，用户点击生成也不会修改目标项目。
